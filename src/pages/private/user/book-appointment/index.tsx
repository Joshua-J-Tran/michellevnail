...
function BookAppointmentPage() {
  ...
  const getData = async () => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Red Alert! Salon Data gone!",
      );
    } finally {
      setLoading(false);
    }
  ...
  const getServices = async (ownerObject: any) => {
    try {
      ...
      const res = await axios.get(
        `${backendUrl}/services/get-services-by-owner-id/${ownerObject}`,
      );
  ...
  useEffect(() => {
    ...
    const fetchWorkers = async () => {
      try {
        const res = await axios.get(
          `${backendUrl}/workers/get-worker-by-salon-id/${params.id}`,
        );
        if (!res.data.success) throw new Error(res.data.message);
        ...
        const selectedMainServices = Object.keys(selectedServices).filter(
        ...
        const filteredWorkers = workers.filter(
          (worker) => worker.isActive && worker.workingDays.includes(dayName),
  ...
  const renderSalonProperty = (label: string, value: string | number) => (
    <div className="flex items-start gap-2 flex-wrap">
      ...
      <h1 className="text-sm text-stone-700 font-semibold break-words">
        {value}
      </h1>
    </div>
  );
  ...
  const handleSubserviceCheck = (serviceId: string, subName: string) => {
    setSelectedSubservices((prev) => ({
      ...
      [serviceId]: {
        ...Object.fromEntries(
          Object.keys(prev[serviceId] || {}).map((name) => [name, false]),
        ),
        [subName]: true,
      },
    }));
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-5">
          ...
          <div className="flex flex-col gap-7 border-2 border-stone-500 rounded-lg p-5 bg-white shadow">
            <div className="flex flex-col gap-3">
              ...
              <div>
                <h1 className="text-sm text-zinc-800 font-bold mb-2">
                  Available Days
                </h1>
                      ...
                      {openDay === day && salonData.schedule?.[day] && (
                        <div className="ml-4 text-sm text-stone-700">
                          <div>
                            ...
                            {formatTimeToAMPM(
                              salonData.schedule[day].startTime,
                            )}
                          </div>
                          <div>
                            <span className="font-bold text-stone-900">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 w-full">
              ...
              <div className="flex flex-col gap-4 mt-2 w-full">
                ...
                <div className="border-2 border-amber-500 rounded-lg p-5 bg-white shadow flex flex-col gap-4 w-full">
                  ...
                  {selectedDate && selectedTime && availableCount !== null && (
                    <div
                      className={`mt-2 text-sm font-semibold ${availableCount > 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {availableCount > 0
                    ...
                    <div className="space-y-2">
                      {ownerServices.map((service) =>
                        Object.entries(service.services).map(
                          ([mainServiceName, subtypes]) => {
                            const serviceKey = service._id + mainServiceName;
                            const isExpanded =
                              expandedServices[serviceKey] ?? true;
                            return (
                              <div
                                key={serviceKey}
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  <button
                                    type="button"
                                    onClick={() => handleExpand(serviceKey)}
                                    className="p-1"
                                  >
                                    {isExpanded ? (
                                      <ChevronDown size={18} />
                                    ...
                                    {subtypes.map((sub) => (
                                      ...
                                      >
                                        <input
                                          type="checkbox"
                                          checked={
                                            !!selectedSubservices[serviceKey]?.[
                                              sub.name
                                            ]
                                          }
                                          disabled={
                                        ...
                                        <span
                                          className={`text-xs ml-2 ${selectedServices[serviceKey] ? "text-gray-500" : "text-gray-300"}`}
                                        >
                                          {sub.description}
                                        </span>
                                      </label>
              ...
              <div className="flex flex-col gap-4 mt-2 w-full">
                  ...
                  Object.keys(selectedSubservices).length === 0 ? (
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const worker =
                          availableWorkers.find((w) => w._id === workerId) ||
                          null;
                ...
                <div className="border-4 border-black rounded-lg p-5 bg-yellow-400 shadow flex flex-col gap-4 w-full">
                  ...
                  If you are trying to book for 3 or more people, please contact
                  us via phone at{" "}
                  <strong className="font-bold">
                    +1{salonData.phonenumber}
                  </strong>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-center mt-10">
            <Button
              className="bg-green-700 text-secondary"
              disabled={
                loading || !selectedDate || !selectedTime || availableCount <= 0
              }
